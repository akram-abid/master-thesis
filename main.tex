...
%	% ----------------------------------------------------------
	...
	\begin{equation}
		...
		\label{eq:lpf}
	\end{equation}
	where $h(x,y)$ is the filter kernel and $\sigma$ controls the 
	degree of blurring. A key property is that in the frequency domain, 
	this convolution becomes a simple multiplication:
	\begin{equation}
		\tilde{F}(u,v) = F(u,v) \cdot H(u,v),
		\label{eq:lpf_freq}
	\end{equation}
	where $H(u,v) \in [0,1]$ is an attenuation factor --- values near 
	zero mean that frequency is heavily suppressed. For a Gaussian blur 
	with $\sigma = 1.5$, mid-frequency components are already reduced 
	by $30$--$60\%$.
